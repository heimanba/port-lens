# ports：告别端口冲突，一键查看谁占了你的端口

起服务报「端口已被占用」？输入 `ports` 瞬间呈现清晰表格——端口、进程、PID、项目路径、技术栈、运行时间一目了然，告别 `lsof`、`pkill` 等复杂命令的记忆负担。
...
- 启动服务时"3000端口被占用"，却不知道是谁占的
- 想用 `lsof` 查端口，却记不住命令参数
- 找到进程后还要手动 `kill`，步骤繁琐

Port Lens 一条命令解决以上所有问题。
